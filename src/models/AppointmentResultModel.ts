...
export class AppointmentResultModel implements IJsonModel {
  ...
  public fromJson(json: any): AppointmentResultModel {
    ...
    this._diagnosis = json.diagnosis
      ? json.diagnosis.map((d: any) => new Diagnosis(d))
      : [];
    this._recommendations = json.recommendations
      ? json.recommendations.map((r: any) => new Procedure().fromJson(r))
      : [];
    this._scheduledProcedures = json.scheduledProcedures
      ? json.scheduledProcedures.map((p: any) => new Procedure().fromJson(p))
      : [];
    this._prescriptions = json.prescriptions
      ? json.prescriptions.map((p: any) => new PrescriptionInfo().fromJson(p))
      : [];
