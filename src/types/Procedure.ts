...
export class Procedure {
  ...
  fromJson(json: any): Procedure {
    ...
    this.services = json.services
      ? json.services.map((s: object) => new Service().fromJson(s))
      : [];
    this.type = json.type;
    this.required = json.required;
    this.status = json.status;

    if (json.period) this.period.fromJson(json.period);

    if (json.strictPeriod) this.strictPeriod.fromJson(json.strictPeriod);
