...
export class AppointmentService extends JsonRPCCredService
    ...
    getAppointments(limit: number, offset: number, cb: (err: any, appointments: AppointmentMessage[]) => void): void {
        ...
        this.exec(Handlers.HANDLER_GET_APPOINTMENTS_METHOD, params, (err: any, payload: object) => {
            if (err)
                return cb(err, null);
    ...
    getAppointmentsAsync(limit: number, offset: number): Promise<AppointmentMessage[]> {
        const service = this;
        return new Promise((res, rej) => {
            service.getAppointments(limit, offset, (err: any, appointments: AppointmentMessage[]) => {
                if (err)
                    return rej(err);
    ...
    getAppointmentsCount(cb: (err: any, count: number, support: boolean) => void): void {
        this.exec(Handlers.HANDLER_GET_APPOINTMENTS_COUNT_METHOD, {}, (err: any, payload: object) => {
            if (err)
    ...
    getAppointmentsCountAsync():Promise<{count: number, support: boolean}> {
        const service = this;
        return new Promise((res, rej) => {
            service.getAppointmentsCount((err: any, count: number, support: boolean) => {
                if (err)
                    return rej(err);
    ...
    getPatientAppointmentsCount(patientId: string, cb: (err: any, count: number, support: boolean) => void): void {
        this.exec(Handlers.HANDLER_GET_PATIENT_APPOINTMENTS_COUNT_METHOD, {patientId}, (err: any, payload: object) => {
            if (err)
                return cb(err, null, false);
    ...
    getPatientAppointmentsCountAsync(patientId: string):Promise<{count: number, support: boolean}> {
        const service = this;
        return new Promise((res, rej) => {
            service.getPatientAppointmentsCount(patientId, (err: any, count: number, support: boolean) => {
                if (err)
                    return rej(err);
