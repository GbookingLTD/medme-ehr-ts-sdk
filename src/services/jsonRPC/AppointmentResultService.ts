...
export class AppointmentResultService extends JsonRPCCredService implements IAppointmentResultService {
     ...
     * @param cb callback
     */
    public getAppointmentResultById(id: string, cb: (err: any, appointmentResult: AppointmentResultMessage) => void): void {
    ...
    public getAppointmentResultByIdAsync(id: string): Promise<AppointmentResultMessage> {
        const service = this;
        return new Promise((res, rej) => {
            service.getAppointmentResultById(id, (err: any, appointment: AppointmentResultMessage) => {
                if (err)
                    return rej(err);
    ...
    getAppointmentResults(limit: number, offset: number,
        ...
        this.exec(Handlers.HANDLER_GET_APPOINTMENT_RESULTS_METHOD, params, (err: any, payload: object) => {
            if (err)
                return cb(err, null);
    ...
    getAppointmentResultsAsync(limit: number, offset: number): Promise<AppointmentResultModel[]> {
        const service = this;
        return new Promise((res, rej) => {
            service.getAppointmentResults(limit, offset, (err: any, appResults: AppointmentResultModel[]) => {
                if (err)
                    return rej(err);
    ...
    getAppointmentResultsCount(cb: (err: any, count: number, support: boolean) => void): void {
        this.exec(Handlers.HANDLER_GET_APPOINTMENT_RESULTS_COUNT_METHOD, {}, (err: any, payload: object) => {
            if (err)
    ...
    getAppointmentResultsCountAsync():Promise<{count: number, support: boolean}> {
        const service = this;
        return new Promise((res, rej) => {
            service.getAppointmentResultsCount((err: any, count: number, support: boolean) => {
                if (err)
                    return rej(err);
    ...
    getPatientAppointmentResultsCount(patientId: string, cb: (err: any, count: number, support: boolean) => void): void {
        this.exec(Handlers.HANDLER_GET_PATIENT_APPOINTMENT_RESULTS_COUNT_METHOD, {patientId}, (err: any, payload: object) => {
            if (err)
                return cb(err, null, false);
    ...
    getPatientAppointmentResultsCountAsync(patientId: string):Promise<{count: number, support: boolean}> {
        const service = this;
        return new Promise((res, rej) => {
            service.getPatientAppointmentResultsCount(patientId, (err: any, count: number, support: boolean) => {
                if (err)
                    return rej(err);
